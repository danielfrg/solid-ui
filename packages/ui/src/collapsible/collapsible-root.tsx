...
import { CollapsibleContext, type CollapsibleContextValue, type CollapsibleDataSet } from "./collapsible-context"
...
export interface CollapsibleRootCommonProps<T extends HTMLElement = HTMLElement> {
  id: string
}

export interface CollapsibleRootRenderProps extends CollapsibleRootCommonProps, CollapsibleDataSet {}

export type CollapsibleRootProps<T extends ValidComponent | HTMLElement = HTMLElement> = CollapsibleRootOptions &
  Partial<CollapsibleRootCommonProps<ElementOf<T>>>

/**
 * An interactive component which expands/collapses a content.
 */
export function CollapsibleRoot<T extends ValidComponent = "div">(props: PolymorphicProps<T, CollapsibleRootProps<T>>) {
  const defaultId = `collapsible-${createUniqueId()}`

  const mergedProps = mergeDefaultProps({ id: defaultId }, props as CollapsibleRootProps)
