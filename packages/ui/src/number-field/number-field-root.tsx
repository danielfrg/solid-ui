...
import type { SpinButtonRootOptions } from "../spin-button/spin-button-root"
import { NumberFieldContext, type NumberFieldContextValue } from "./number-field-context"

export interface NumberFieldRootOptions extends Pick<SpinButtonRootOptions, "textValue" | "translations"> {
...
export interface NumberFieldRootCommonProps<T extends HTMLElement = HTMLElement> {
  id: string
  ref: T | ((el: T) => void)
}

export interface NumberFieldRootRenderProps extends NumberFieldRootCommonProps, FormControlDataSet {
  role: "group"
}

export type NumberFieldRootProps<T extends ValidComponent | HTMLElement = HTMLElement> = NumberFieldRootOptions &
  Partial<NumberFieldRootCommonProps<ElementOf<T>>>

/**
 * A text input that allow users to input custom text entries with a keyboard.
 */
export function NumberFieldRoot<T extends ValidComponent = "div">(props: PolymorphicProps<T, NumberFieldRootProps<T>>) {
  ...
  const parseRawValue = (value: string | number | undefined) =>
    local.format && typeof value !== "number" ? numberParser().parse(value ?? "") : Number(value ?? "")
  ...
  const context: NumberFieldContextValue = {
    ...
    varyValue: (offset) => {
      ...
      batch(() => {
        ...
        const min = props.minValue === undefined ? Number.NaN : context.minValue()
        const max = props.maxValue === undefined ? Number.NaN : context.maxValue()
