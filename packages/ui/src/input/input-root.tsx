...
export interface InputRootRenderProps extends InputRootCommonProps, FormControlDataSet {
  role: "group"
}

export type InputRootProps<T extends ValidComponent | HTMLElement = HTMLElement> = InputRootOptions &
  Partial<InputRootCommonProps<ElementOf<T>>>

/**
 * A text input that allow users to input custom text entries with a keyboard.
 */
export function InputRoot<T extends ValidComponent = "div">(props: PolymorphicProps<T, InputRootProps<T>>) {
  let ref: HTMLElement | undefined

  const defaultId = `input-${createUniqueId()}`

  const mergedProps = mergeDefaultProps({ id: defaultId }, props as InputRootProps)
