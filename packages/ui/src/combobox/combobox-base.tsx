...
import { ComboboxContext, type ComboboxContextValue, type ComboboxDataSet } from "./combobox-context"
import { COMBOBOX_INTL_TRANSLATIONS, type ComboboxIntlTranslations } from "./combobox.intl"
...
export interface ComboboxBaseOptions<Option, OptGroup = never> extends Omit<
  PopperRootOptions,
...
export interface ComboboxBaseRenderProps extends ComboboxBaseCommonProps, FormControlDataSet, ComboboxDataSet {
  role: "group"
}
...
> = ComboboxBaseOptions<Option, OptGroup> & Partial<ComboboxBaseCommonProps<ElementOf<T>>>

/**
 * Base component for a combobox, provide context for its children.
 */
export function ComboboxBase<Option, OptGroup = never, T extends ValidComponent = "div">(
  ...
  const getOptionValue = (option: Option) => {
    ...
    return String(isFunction(optionValue) ? optionValue(option as any) : (option as any)[optionValue])
  }
  ...
  const getOptionLabel = (option: Option) => {
    ...
    return String(isFunction(optionLabel) ? optionLabel(option as any) : (option as any)[optionLabel])
  }
  ...
  const getOptionTextValue = (option: Option) => {
    ...
    // Get the label from the option object as a string.
    return String(isFunction(optionTextValue) ? optionTextValue(option as any) : (option as any)[optionTextValue])
  }
  ...
  const allOptions = createMemo(() => {
    ...
    return local.options!.flatMap((item) => ((item as any)[optionGroupChildren] as Option[]) ?? (item as Option))
  })
  ...
  const filteredOptions = createMemo(() => {
    ...
    for (const optGroup of local.options as OptGroup[]) {
      ...
      const filteredChildrenOptions = ((optGroup as any)[optionGroupChildren] as Option[]).filter(filterFn)
