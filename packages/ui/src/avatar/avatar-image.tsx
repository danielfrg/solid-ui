...
import { Show, type ValidComponent, createEffect, createSignal, on, onCleanup } from "solid-js"
...
export type AvatarImageProps<T extends ValidComponent | HTMLElement = HTMLElement> = AvatarImageOptions &
  Partial<AvatarImageCommonProps<ElementOf<T>>>

/**
 * The image to render. By default, it will only render when it has loaded.
 */
export function AvatarImage<T extends ValidComponent = "img">(props: PolymorphicProps<T, AvatarImageProps<T>>) {
  const context = useAvatarContext()

  const [loadingStatus, setLoadingStatus] = createSignal<AvatarLoadingStatus>("idle")
  ...
  return (
    <Show when={loadingStatus() === "loaded"}>
      <Polymorphic<AvatarImageRenderProps> as="img" {...(props as AvatarImageProps)} />
    </Show>
  )
}
