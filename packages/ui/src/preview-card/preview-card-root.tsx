...
import { PreviewCardContext, type PreviewCardContextValue, type PreviewCardDataSet } from "./preview-card-context"
import { getPreviewCardSafeArea } from "./utils"

export interface PreviewCardRootOptions extends Omit<
  PopperRootOptions,
...
> {
  ...
  /** Event handler called when the open state of the preview card changes. */
  onOpenChange?: (isOpen: boolean) => void

  /** The duration from when the mouse enters the trigger until the preview card opens. */
  openDelay?: number

  /** The duration from when the mouse leaves the trigger or content until the preview card closes. */
  closeDelay?: number

  /** Whether to close the preview card even if the user cursor is inside the safe area between the trigger and preview card. */
  ignoreSafeArea?: boolean

  /**
   * Used to force mounting the preview card (portal and content) when more control is needed.
...
export interface PreviewCardRootProps extends ParentProps<PreviewCardRootOptions> {}
...
export function PreviewCardRoot(props: PreviewCardRootProps) {
  const defaultId = `previewcard-${createUniqueId()}`
  ...
  const isTargetOnPreviewCard = (target: Node | null) => {
  ...
  const getPolygonSafeArea = (placement: Placement) => {
    ...
    return getPreviewCardSafeArea(placement, triggerEl, contentEl)
  }

  const onHoverOutside = (event: PointerEvent) => {
    const target = event.target as Node | null

    // Don't close if the mouse is moving through valid preview card element.
    if (isTargetOnPreviewCard(target)) {
      cancelClosing()
      return
    }
    ...
    // If there's already a scheduled timeout to hide the preview card, we do nothing.
    if (closeTimeoutId) {
      return
    }

    // Otherwise, hide the preview card after the close delay.
    closeWithDelay()
  }
  ...
  createEffect(() => {
    ...
    // Checks whether the mouse is moving outside the preview card.
  ...
  const dataset: Accessor<PreviewCardDataSet> = createMemo(() => ({
