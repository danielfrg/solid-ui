...
export function DemoToggleGroupControlled() {
  const [value, setValue] = createSignal("bold")

  return (
    <div>
      <ToggleGroup.Root value={value()} onChange={setValue} class={styles.panel}>
        <ToggleGroup.Item aria-label="Bold" value="bold" class={styles.button}>
          <BoldIcon class={styles.icon} />
        </ToggleGroup.Item>
        <ToggleGroup.Item aria-label="Italic" value="italic" class={styles.button}>
          <ItalicIcon class={styles.icon} />
        </ToggleGroup.Item>
        <ToggleGroup.Item aria-label="Underline" value="underline" class={styles.button}>
          <UnderlineIcon class={styles.icon} />
        </ToggleGroup.Item>
      </ToggleGroup.Root>
      <p class={styles.status}>Active: {value() || "none"}</p>
    </div>
  )
}
